...
async def test_post_log_exitoso(client, log_valido):
    ...
    assert "id" in data          # _id serializado
    assert "timestamp" in data   # calculado por la API
    assert "expires_at" in data  # calculado por la API según tipo
...
async def test_post_log_tipo_invalido(client):
    datos = {
        "type": "PAGOS",  # no existe en el Literal
        "severity": "INFO",
...
async def test_post_log_campo_faltante(client):
    sin_type = {
        "severity": "INFO",
        "service": "auth-service",
        # falta: type, correlation_id, user_id, detail
    }
...
async def test_post_log_cliente_no_puede_inyectar_expires_at(client, log_valido):
    ...
    # El endpoint acepta el payload (Pydantic ignora campos extra)
    # y devuelve el expires_at calculado internamente, no el inyectado
    assert response.status_code == 201
    # El expires_at en la respuesta viene del mock (calculado), no del cliente
